<div>
    ...
    <div class="space-y-4">
                ...
                <tbody class="[&>tr:nth-child(odd)]:bg-white [&>tr:nth-child(even)]:bg-zinc-50">
                    @forelse($workOrders as $workOrder)
                        <tr class="border-b border-zinc-200 hover:bg-zinc-50">
                            ...
                            <td class="px-2 py-1 whitespace-nowrap">{{ $workOrder->instalador?->nombre_instalador ?? '' }}</td>
                            ...
                            <td class="px-2 py-1">
                                @if($isInstalador)
                                    ...
                                    <div class="flex justify-center gap-2">
                                        <button wire:click="openEditWorkOrder({{ $workOrder->id_work_order }})" type="button"
                                            class="btn btn-outline-secondary btn-sm">
                                            Iniciar
                                        </button>

                                        <button wire:click="openViewWorkOrder({{ $workOrder->id_work_order }})" type="button"
        ...
        <div class="pt-2 text-xs">{{ $workOrders->links() }}</div>
